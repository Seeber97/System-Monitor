...
long LinuxParser::ActiveJiffies(int pid) { 
  ...
  if(stream.is_open()){
    ...
    for(int i=0 ; i<17 ; i++){
      ...
      if(i== 14){ stime = stof(value) ;}
      if(i== 15){ cutime = stof(value) ;}
      if(i== 16){ cstime = stof(value) ;}
    }
...
float LinuxParser::process_cpuutilization(int pid) { 
  ...
  float seconds = static_cast<long>( LinuxParser::UpTime(pid));
  
     Totaltime = 100*(Totaltime / seconds) ;
